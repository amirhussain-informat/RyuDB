...
// drop also invalidates the engine scan cache, so a SELECT on the dropped name
// errors (it would otherwise return the stale cached frame).
const droppedQ = await call(ws, { id: "dq", op: "sql", sql: "SELECT count(*) AS c FROM li_copy" });
check("dropped table unqueryable", droppedQ.meta.op === "error", JSON.stringify(droppedQ.meta));
...
// (Admin drop/rename now invalidate the engine scan cache — see the
